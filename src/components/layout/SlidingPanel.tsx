        ...
        <div 
            className={`fixed bottom-0 border-t border-gray-200 left-0 right-0 bg-white transition-all duration-300 ease-in-out h-[80vh] transform ${
                isExpanded 
                    ...
                    : 'translate-y-[calc(100%-44px)] shadow-[0_-3rem_3rem_-3rem_rgba(0,0,0,0.25)]'
            }`}
            style={{ zIndex: 1000 }}
        >
            {hasNotification && !isExpanded && (
                <div className="bg-blue-500 w-4 h-4 rounded-full absolute top-3.5 left-1/2 -translate-x-8 z-10 animate-pulse"></div>
            )}
                <button
                    onClick={() => {setIsExpanded(!isExpanded); setHasNotification?.(false);}}
                    className="absolute top-0 left-1/2 -translate-x-1/2 cursor-pointer hover:bg-gray-200 p-2 w-full flex justify-center border-b border-gray-200 bg-white"
            >
                <ChevronIcon 
